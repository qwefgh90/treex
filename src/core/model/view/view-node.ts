...
// export class ViewNodeImpl<T> implements ViewNode<T>{
//     constructor(private node: Node | NodeProp){
//     }
// }
...
export interface ViewNode<T> extends RootNode<T>{
    ...
    bfs(predicate: (entry: {item: TreeNode<T>, level: number}) => boolean, stopWhenFound: boolean): {item: TreeNode<T>, level: number}[];
}
